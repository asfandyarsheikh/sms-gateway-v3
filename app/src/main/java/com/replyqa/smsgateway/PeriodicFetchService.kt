...
class PeriodicFetchService : Service() {
    ...
    private suspend fun fetchAndProcessSms(config: AppConfig) {
        try {
            ...
            // Add authorization header using when expression to avoid compilation issues
            val authHeader = config.authorization
            when {
                authHeader.isNotEmpty() -> requestBuilder.header("Authorization", authHeader)
            }
